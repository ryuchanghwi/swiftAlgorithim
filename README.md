...
| ------ | ---- |----------- |
| 23.01.07 | 1.| 1. - |
| 23.01.03 | 1.잘라서 배열로 저장하기 | 1. - |
| 22.12.18 | 1.정수내림차순으로 배치하기 | 1. 배열을 만든후 sorted(by: >)를 이용해 내림차순으로 만든 값들을 하나씩 문자열로 더해준다. |
| 22.12.17 | 1.종이 자르기 | 1. 문제를 이해하는데 많은 시간이 걸렸지만, 풀이는 한 번 크게 자르고 나머지를 넓이가 1이 되게끔 만들어야 하기 떄문에 M * N - 1로 구한다.   |
| 22.12.15 | 1.직사각형 넓이 구하기 | 1. 이중 배열을 통해서 x값과 y을 구한다.  |
| 22.12.12 | 1.최솟값 만들기 | 1. 2개의 배열을 하나는 큰 수부터, 하나는 작은 수부터 나열한 후, 곱한다. |
| 22.12.11 | 1.소인수분해 | 1. while문과 for문을 통해 소인수분해를 진행, break를 통해 조건을 만족하면 while문  |
| 22.12.10 | 1.모의고사 | 1. 반복되는 수를 배열에 담고 개수 % 인덱스를 통해 반복되는 값에 접근  |
| 22.12.09 | 1.과일 장수 | 1. for문을 돌리며, 박스에 채워져야 하는 갯수를 카운트 하고 조건에 맞을 경우만 반환   |
| 22.12.08 | 1.가운데 글자 가져오기 | 1. 문자열을 배열로 만들고, 삼항 연산 조건문을 통해, 가운데 수를 반환  |
